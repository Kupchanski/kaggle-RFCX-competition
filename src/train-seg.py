...
class Model(pl.LightningModule):
    ...
    def __init__(self, arch, n_classes=24, learning_rate=3e-4, mixup_p=0., y_coord=False, num_epochs=30):
        ...
        self.decoder = smp.unet.model.UnetDecoder(self.model.out_channels, [256, 128, 64, 32, 16])
        self.seg_head = smp.base.heads.SegmentationHead(16, 1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_pred, seg = self._forward(images)
        
        # classification loss
        cls_loss = lsep_loss_stable(y_pred, y_true)
        # loss = focal_loss(y_pred, y_true)
        # loss = 100 * focal_loss(y_pred, y_true) + lsep_loss_stable(y_pred, y_true)

        # segmentation loss
        seg_gt = (batch["mask"] >= 0).to(dtype=torch.float)
        seg_pr = seg.squeeze(0)
        seg_loss_fn = smp.losses.DiceLoss(mode="binary", smooth=1.)
        seg_loss = seg_loss_fn(seg_pr, seg_gt)
        
        loss = cls_loss + seg_loss

        self.log("dice", 1 - seg_loss, prog_bar=True)
        return dict(loss=loss)

    def validation_step(self, batch, batch_idx):
        images = batch["image"].float()
        y_true = batch["label"]
        y_pred, _ = self._forward(images)
